import { mkdir, readFile, writeFile } from 'node:fs/promises';
import path from 'node:path';

import { JSDOM } from 'jsdom';
import JSZip from 'jszip';

import type { BlockNode, ChatDocument, InlineNode } from '../src/document/ast';
import type { ExportRequest } from '../src/document/export';
import { parseMessageContentResult } from '../src/platform/chatgpt/content-parser';
import { renderStructuredDocx } from '../src/renderers/docx/docx-node-renderer';

function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(message);
}

const fixturePath = path.resolve('tests/fixtures/chatgpt/m13-actual-parity-shapes.html');
const dom = new JSDOM(await readFile(fixturePath, 'utf8'), {
  url: 'https://chatgpt.com/c/m13-actual-parity',
});
const element = dom.window.document.querySelector<HTMLElement>(
  '[data-message-id="m13-actual-parity"]',
);
assert(element, 'M13.5 actual-shape fixture message is missing.');
const parsed = parseMessageContentResult(element, 'm13-actual-parity');

const inlineSources = ['q=3', '\\Delta t_{gap}', 'W_{gap}', 'x_u(t)'];
const inlineChildren: InlineNode[] = [{ kind: 'text', value: 'Additional inline coverage: ' }];
inlineSources.forEach((source, index) => {
  inlineChildren.push({
    fallbackText: source,
    kind: 'mathInline',
    provenance: 'explicit',
    source,
    sourceFormat: 'tex',
  });
  if (index < inlineSources.length - 1) inlineChildren.push({ kind: 'text', value: ', ' });
});
inlineChildren.push({ kind: 'text', value: '.' });

const displaySources = [
  '\\frac{a}{b}',
  'x_i^2',
  '\\sqrt[3]{x}',
  '\\sum_{i=1}^{n} i',
  '\\int_0^1 x\\,dx',
  '\\left( a+b \\right)',
  '\\begin{bmatrix}1 & 2 \\\\ 3 & 4\\end{bmatrix}',
  '\\alpha + \\beta \\le \\infty',
  '\\mathcal{T}_m^\\alpha \\rightarrow R_{UA} \\subseteq Z_{int}',
  '\\begin{pmatrix}a & b \\\\ c & d\\end{pmatrix}',
  'q=3',
  '\\Delta t_{gap}',
  'v_u(t)',
  'a_u(t)=\\{release,hold,v_u(t)\\}',
  'R_{UA}',
  'Z_{int}',
  'T_m^\\alpha',
  'W_{gap}',
  'x_u(t)=(c_u,s_u,v_u,q_u)',
];
const displayBlocks: BlockNode[] = displaySources.map((source) => ({
  fallbackText: source,
  kind: 'mathBlock',
  provenance: 'explicit',
  source,
  sourceFormat: 'tex',
}));
parsed.content.push({ children: inlineChildren, kind: 'paragraph' }, ...displayBlocks);

const document: ChatDocument = {
  version: 1,
  title: 'M13.5 Sanitized Actual-Shape DOCX Fixture',
  source: {
    platform: 'chatgpt',
    url: 'https://chatgpt.com/c/m13-actual-parity',
    capturedAt: '2026-06-29T07:00:00.000Z',
  },
  exportedAt: '2026-06-29T07:01:00.000Z',
  messages: [{
    content: parsed.content,
    id: 'm13-actual-parity',
    order: 0,
    role: 'assistant',
    selected: true,
    status: 'complete',
  }],
  warnings: parsed.warnings,
};
const request: ExportRequest = {
  document,
  selection: { scope: 'single-response', messageId: 'm13-actual-parity' },
  options: {
    codeStyle: 'document',
    fileName: 'm13-5-docx-parity',
    format: 'docx',
    includePrompts: true,
    language: 'en',
    paper: 'a4',
    theme: 'light',
  },
};

const result = await renderStructuredDocx(request);
const archive = await JSZip.loadAsync(await result.blob.arrayBuffer());
const documentXml = await archive.file('word/document.xml')!.async('string');
const mathCount = documentXml.match(/<m:oMath>/gu)?.length ?? 0;
const displayParagraphs = documentXml.match(/<m:oMathPara>[^]*?<\/m:oMathPara>/gu) ?? [];
const textFigureParagraphs = (documentXml.match(/<w:p(?:\s[^>]*)?>[^]*?<\/w:p>/gu) ?? [])
  .filter((paragraph) => paragraph.includes('w:pStyle w:val="ChatExportTextFigure"'));
const textFigureXml = textFigureParagraphs.join('');
const textFigureMathCount = textFigureXml.match(/<m:oMath>/gu)?.length ?? 0;

assert(mathCount === 31, `Expected 31 editable equations, received ${mathCount}.`);
assert(displayParagraphs.length === 20,
  `Expected 20 display equations, received ${displayParagraphs.length}.`);
assert(displayParagraphs.some((paragraph) =>
  ['Z', '↓', 'q', '3'].every((value) => paragraph.includes(value))),
'Compact Zint display formula is not native OMML.');
assert(!/<w:t[^>]*>[^<]*τ∈\[0,H\]/u.test(documentXml),
  'Tau interval leaked as plain Word text.');
assert(textFigureParagraphs.length === 2,
  `Expected two text figures, received ${textFigureParagraphs.length}.`);
assert(textFigureMathCount === 4,
  `Expected four anchored text-figure equations, received ${textFigureMathCount}.`);
for (const text of ['蓝色椭圆', '绿色矩形', '┌───────────┐', 'control action', '→ ‖ ']) {
  assert(textFigureXml.includes(text), `Text-figure content is missing: ${text}`);
}
for (const text of ['T_m^α', 'Z_int', 'R_UA', 'v_u(t)']) {
  assert(!textFigureXml.includes(text), `Text-figure math leaked as plain text: ${text}`);
}
assert(documentXml.includes('𝒵'), 'Explicit calligraphic source was not preserved.');
assert(result.warnings.length === 0,
  `Expected no DOCX warnings, received: ${result.warnings.map(({ code }) => code).join(', ')}`);

const outputDir = path.resolve('docs/qa-artifacts/m13-5');
await mkdir(outputDir, { recursive: true });
const outputPath = path.join(outputDir, 'm13-5-docx-parity.docx');
const reportPath = path.join(outputDir, 'm13-5-report.json');
await writeFile(outputPath, new Uint8Array(await result.blob.arrayBuffer()));
await writeFile(reportPath, `${JSON.stringify({
  displayMathParagraphs: displayParagraphs.length,
  file: path.relative(process.cwd(), outputPath),
  mathObjects: mathCount,
  plainTauTextRuns: documentXml.match(/<w:t[^>]*>[^<]*τ∈\[0,H\]/gu)?.length ?? 0,
  size: result.blob.size,
  textFigureMathObjects: textFigureMathCount,
  textFigureParagraphs: textFigureParagraphs.length,
  warningCodes: result.warnings.map(({ code }) => code),
}, null, 2)}\n`);
process.stdout.write(await readFile(reportPath, 'utf8'));

...
const version = '1.0.0';
const releaseLabel = process.argv[2] ?? `v${version}-rc.6`;
const fixedDate = new Date(process.argv[3] ?? '2026-06-29T00:00:00.000Z');
...
for (const target of targets) {
  ...
  packages.push({
    ...
    sha256: sha256(first),
    sourceFileCount: (await listFiles(source)).length,
  });
}

await writeFile(
  path.join(releaseDir, 'SHA256SUMS.txt'),
  `${packages.map((item) => `${item.sha256}  ${path.basename(item.file)}`).join('\n')}\n`,
  'utf8',
);

const report = {
  build: { date: fixedDate.toISOString(), releaseLabel, version },
  packages,
  summary: {
